...
static void
block_upload_client_done_cb (BlockTxInfo *info)
{
    switch (info->result) {
    ...
    case BLOCK_CLIENT_SERVER_ERROR:
        ...
        if (++info->n_failure <= 3) {
            ...
            if (block_tx_client_start (info, block_upload_client_done_cb) < 0) {
...
static void
start_block_upload_client (TransferTask *task)
{
    BlockTxInfo *info;
...
on_getcs_v2_done (CcnetProcessor *processor, gboolean success, void *data)
{
    ...
    if (success) {
        start_block_upload_client (task);
    } else if (task->state != TASK_STATE_ERROR) {
        transfer_task_with_proc_failure (
